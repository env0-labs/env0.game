...
namespace Env0.Act3.Config
{
    public static class JsonLoader
    {
        ...
        public static Dictionary<string, Env0.Act3.Config.Pocos.Filesystem> Filesystems { get; private set; } = new Dictionary<string, Env0.Act3.Config.Pocos.Filesystem>();
        ...
        public static void LoadAll()
        {
            ...
            BootConfig = LoadBootConfig(
                ResolvePath(
                    ...
                    "Env0.Act3/Config/Jsons/BootConfig.json"
                ),
                out var bootErrors
            );
            ...
            UserConfig = LoadUserConfig(
                ResolvePath(
                    ...
                    "Env0.Act3/Config/Jsons/UserConfig.json"
                ),
                out var userErrors
            );
            ...
            Devices = LoadDevices(
                ResolvePath(
                    ...
                    "Env0.Act3/Config/Jsons/Devices.json"
                ),
            ...
            for (int i = 1; i <= 10; i++)
            {
                var path = ResolvePath(
                    ...
                    $"Env0.Act3/Config/Jsons/JsonFilesystems/Filesystem_{i}.json"
                );
            ...
            var safePath = ResolvePath(
                ...
                "Env0.Act3/Config/Jsons/JsonFilesystems/Filesystem_11.json"
            );
        ...
        public static Env0.Act3.Config.Pocos.Filesystem LoadFilesystem(string path, out List<string> errors)
        {
            errors = new List<string>();
            ...
            if (!File.Exists(path))
            {
                ...
                return new Env0.Act3.Config.Pocos.Filesystem(); // Safe empty FS
            }

            try
            {
                var json = File.ReadAllText(path);
                var fs = JsonConvert.DeserializeObject<Env0.Act3.Config.Pocos.Filesystem>(json);

                if (fs?.Root == null || fs.Root.Count == 0)
                {
                    errors.Add($"{Path.GetFileName(path)}: Root is missing or empty.");
                    return new Env0.Act3.Config.Pocos.Filesystem();
                }
